...
namespace UserGroups.Application.IntegrationTests.UserGroups.Presenters.Queries
{
    ...
    public class QueryPresenterTests : TestBase
    {
        ...
        public async Task ShouldFilterPresenters()
        {
            SetRoles(new List<ApplicationRoles> { ApplicationRoles.Admin });
            for (var i = 0; i < 10; i++)
                await CreateTestPresenter($"Test ${i} Blurb", $"Test ${i} Short Bio", deleted: false);

            await CreateTestPresenter("Find Me ", bio: "Test  Short Blurb", deleted: false);
            await CreateTestPresenter("Test Blurb", bio: "Find Me", deleted: false);
        ...
        {
            SetRoles(new List<ApplicationRoles> { ApplicationRoles.Admin });
            await CreateTestPresenter("Test Blurb", "Test Short Blurb", deleted: true);
