...
namespace UserGroups.Application.IntegrationTests.TestData
{
    ...
    public static class TestDataManager
    {
        ...
        public static async Task<Meeting> CreateTestMeeting(string createAsUserId, bool isDeleted = false, bool isDraft = false,
                                                            int maxRsvp = 0, bool allowRsvp = true, bool isPastEvent = false, bool isFuturePublish = false)
        {
            var x = await FindAsync<OmahaMtgUser>(createAsUserId);
            return await AddAsync(
                new Faker<Meeting>()
                    ...
                    .RuleFor(h => h.CreatedByUserId, b => createAsUserId)
                    .RuleFor(h => h.UpdatedByUserId, b => createAsUserId)
                    .Generate()
            );
        }
